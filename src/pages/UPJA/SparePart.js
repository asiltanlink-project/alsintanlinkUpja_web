...
class Sparepart extends React.Component {
  ...
  getAllAlsin() {
    ...
    fetch(url, option)
      ...
      .then(data => {
        ...
        // var message = data.result.message;
        console.log('Spare part DATA', data);
        if (status === 0) {
  ...
  render() {
      ...
      alsinTodos.map((todo, i) => {
        return (
          <tr key={i}>
            ...
            <td style={{ textAlign: 'left' }}>{todo.spare_part_name}</td>
            <td style={{ textAlign: 'left' }}>{todo.spare_part_type_name}</td>
    ...
    return (
      <Page
        title="Suku Cadang"
        breadcrumbs={[{ name: 'Suku Cadang', active: true }]}
        className="SparePart"
      >
        <Row>
          <Col>
