...
class SpdoPage extends React.Component {
	...
	getBatchList = (procod, outcodeGudang) => {
		...
		fetch(url, req)
			...
			.catch(error => {
				...
				setTimeout(() => {
					this.getBatchList(procod, outcodeGudang)
				}, 1000)
			});
	};
	...
	editBatchEditToggle() {
		const editBatch_collapseEditIsOpen = this.state
			.editBatch_collapseEditIsOpen;
		this.setState({
			editBatch_batchBaru: this.state.editBatch_batchLama,
			editBatch_qtyBaru: 0,
						...
						}
						{
                          ...
                          produk ke urutan paling atas
											...
											{/* <Button className="mx-3" disabled={this.state.isLoading}><MdAdd /> ADD DO</Button>
										{
											...
											{/* <Button className="mx-3" disabled={this.state.isLoading || this.state.allQtyIsMultiple.includes(false)} color='success' onClick={() => this.buttonConfirmDetailSPClick()}><MdDone /> CONFIRM</Button> */}
											<Button
												className="mx-3"
												disabled={this.state.isLoading}
												color="success"
					...
					<Collapse isOpen={this.state.editBatch_collapseEditIsOpen}>
						<ModalBody>
							<Form>
								...
								<Row form className="d-flex justify-content-center">
									{/* <Input disabled={this.state.isLoading} className='w-50 text-center' value={this.state.editBatch_batchBaru} onChange={event => this.handleInputBatchNewOnChange(event)} /> */}
									<Input
										type="select"
										disabled={this.state.isLoading}
										className="w-50 text-center"
									...
									>
										{this.state.batchList.map((batch, index) => (
											<option key={index} value={batch['stck_batchnumber']}>
					...
					<ModalFooter className={'d-flex justify-content-end'}>
						...
						{this.state.editBatch_collapseEditIsOpen && (
							<Button
								disabled={
									this.state
										.isLoading /*|| this.state.editBatch_qtyBaru === 0*/ ||
									!this.state.editBatch_isMedunitMultiplication ||
									this.state.editBatch_batchBaru.length === 0 ||
									this.state.editBatch_batchBaru ===
									this.state.editBatch_batchLama
								}
								color="success"
								onClick={() => {
									this.setState(
