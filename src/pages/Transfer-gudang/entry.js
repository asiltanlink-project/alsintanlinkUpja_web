...
import {  Table, Label, Input , Form ,FormFeedback, Card, CardHeader,CardBody, 
...
class Entry extends React.Component{
    ...
    getAllDataKaryawan = () =>{
        const url = (base_url_all+'masterwfh?page='+this.state.page+'&length='+this.state.length)
        Axios
        .get(url,{
            headers:{
    ...
    searchKaryawanByNIP = () => {
        const url = (base_url_all+'masterwfh?NIP='+this.state.inputSearchNIPHome)
        Axios
        .get(url,{
            headers:{
                Authorization:this.state.token
    ...
    searchKaryawanByDept = () =>{
        const url = (base_url_all+'masterwfh?dept='+this.state.selectedDept)
        Axios
        .get(url,{
            headers:{
                Authorization:this.state.token
    ...
    addKaryawanWFH = () =>{
        const url= base_url_all+'masterwfh'
        Axios
        .post(url,{
        ...
        .then((res)=>{
            if(res.data.error.status === false){
                ...
                this.setState({
                    ...
                    ResponseModalMessage:res.data.error.msg
                },()=>this.toggleAllModal("Add"))
            }
            this.getAllDataKaryawan()
        })
    ...
    editKaryawanWFH = () =>{
        ...
        .then((res)=>{
            if(res.data.error.status === false){
                this.setState({
                    ResponseModalHeader:'Success',
                    ResponseModalMessage:'Berhasil Mengubah Data'
                },()=>this.toggleAllModal("Edit"))
            }else if(res.data.error.status === true){
                this.setState({
                    ResponseModalHeader:'Failed',
                    ResponseModalMessage:'Gagal Mengubah Data'
                },()=>this.toggleAllModal("Edit"))
            }
            this.getAllDataKaryawan()
        })
    ...
    validateJumlahKerja = () =>{
        if(this.state.inputJumlahKerja < 0 || this.state.inputJumlahKerja > 5 
    ...
    openConfirmationModal = (data) => {
       if(this.state.openedModalType === "Add"){
        this.setState({
            inputNIP:data[0].nip,
            inputNama:data[0].nama,
            inputPT:data[0].pt,
            inputDept:data[0].departemen,
    ...
    render(){
        const {btnSearchTypeIsOpen,btnModalSearchTypeIsOpen,btnPaginationAmountIsOpen,
            searchType,modalSearchType,paginationAmount,
            result,resultModal,resultPT,resultDept,ResponseModalHeader,ResponseModalMessage} = this.state
        return(
           <Page
           title       = "Entry Data WFH"
           breadcrumbs = {[{ name: 'Entry Data WFH', active: true }]}
           className   = "entry"
           >
                <Card>
                    <CardHeader>
                    <ButtonDropdown   isOpen={btnPaginationAmountIsOpen} toggle={()=>this.togglePaginationAmount()} style={{float:'left', marginBottom:'1%' ,marginRight:'1%'}}>
                    ...
                    <ButtonDropdown   isOpen={btnSearchTypeIsOpen} toggle={()=>this.toggleSearchButton()} style={{float:'left', marginBottom:'1%' ,marginRight:'1%'}}>
                            ...
                            <DropdownItem hidden={this.state.searchDefault} onClick={(e)=>this.handleSearchHome("default")}>Tampilkan Semua</DropdownItem>
                                ...
                                <DropdownItem hidden={this.state.searchByNIP}  onClick={(e)=>this.handleSearchHome("NIP")}>NIP</DropdownItem>
                            </DropdownMenu>
                        </ButtonDropdown>
                        </Col>
                        ...
                        <Table responsive>
                            ...
                            <tbody>
                                ...
                                <tr key={emp.nip}>
                                    ...
                                    <td style={{width:'20%' , textAlign:'left'}}>{emp.nama}</td>
                                    <td style={{width:'20%' , textAlign:'left'}}>{emp.pt}</td>
                                    <td style={{width:'25%' , textAlign:'left'}}>{emp.departemen}</td>
                                    <td style={{width:'10%' , textAlign:'center'}}>{emp.hk_masuk_perminggu}</td>
                ...
                <Modal isOpen={this.state.modalAddIsOpen}  size='lg'>
                    ...
                    <Row>
                        <Col xs={2} md={2}>
                    <ButtonDropdown   isOpen={btnModalSearchTypeIsOpen} toggle={()=>this.toggleSearchButton("Modal")} style={{float:'left', marginBottom:'1%' ,marginRight:'1%'}}>
                            <DropdownToggle caret>
                                {modalSearchType}
                            </DropdownToggle>
                        </ButtonDropdown>
                        ...
                        <Input 
                        style={{width:'100%'}}
                        onChange={(e)=>this.handleChange("inputSearchNIPModal",e)}
                        maxLength={7}
                        valid={this.state.validSearchModal}
                        invalid={this.state.invalidSearchModal}
                        ></Input>
                         {this.state.invalidSearchModal && <FormFeedback>
                            NIP merupakan alphanum sepanjang 6-7 karakter dan tidak boleh mengandung spasi atau karakter spesial
                            </FormFeedback>}
                        </Col>
                        <Col xs={2} md={2}>
                        <Button  size ="md" 
                        style={{float:'right', width:'68%'}} 
                        disabled={this.state.btnSearchModalIsDisabled}
                        onClick={()=>this.searchKaryawanToAddByNIP()}
                        >CARI</Button>
                        </Col>
                        </Row>
                        ...
                        <Table style={{marginTop:'2%'}}>
                            <thead>
                                <th style={{width:'20%',textAlign:'left'}}>NIP</th>
                                <th style={{width:'20%',textAlign:'left'}}>Nama</th>
                                <th style={{width:'30%',textAlign:'left'}}>PT</th>
                                <th style={{width:'30%',textAlign:'left'}}>Departemen</th>
                            </thead>
                            <tbody>
                            {resultModal && resultModal.map((emp)=>
                                <tr>
                                    <td style={{width:'20%' , textAlign:'left'}}>{emp.nip}</td>
                                    <td style={{width:'20%' , textAlign:'left'}}>{emp.nama}</td>
                                    <td style={{width:'30%' , textAlign:'left'}}>{emp.pt}</td>
                                    <td style={{width:'30%' , textAlign:'left'}}>{emp.departemen}</td>
                                </tr>
                                )}
                            </tbody>
                        </Table>
                ...
                <Modal isOpen = {this.state.modalEditIsOpen}>
                    ...
                    <ModalBody>
                        ...
                        ></Input>
                        {this.state.invalidJumlahKerja && <FormFeedback>
                            Jumlah Hari Kerja Minimum 2 Hari dan Maksimum 6 Hari Serta Harus Berbeda Dengan Jumlah Sebelumnya
                            </FormFeedback>}
                    </ModalBody>
                    <ModalFooter style={{justifyContent:'center'}}>
                        <Button 
                ...
                <Modal isOpen={this.state.modalConfirmationIsOpen}>
                    ...
                    <Row style={{marginBottom:'1%'}}>
                        <Col xs={3}> <Label>NIP</Label></Col>
                        <Col><Input disabled value={this.state.inputNIP}></Input></Col>
                    </Row>
                    <Row style={{marginBottom:'1%'}}>
                        <Col xs={3}><Label>Nama</Label></Col>
                        <Col><Input disabled value={this.state.inputNama}></Input></Col>
                    </Row>
                    <Row style={{marginBottom:'1%'}}>
                        <Col xs={3}><Label>PT</Label></Col>
                        <Col><Input disabled value={this.state.inputPT}></Input></Col>
                    </Row>
                    <Row style={{marginBottom:'1%'}}> 
                        <Col xs={3} ><Label>Departemen</Label></Col>
                        <Col><Input disabled value={this.state.inputDept}></Input></Col>
                    </Row>
                    <Row style={{marginBottom:'1%'}}>
